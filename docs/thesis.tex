...
Głównym źródłem wiedzy autora na temat mapowania obiektowo-relacyjnego była książka ,,Hibernate w akcji'' napisana przez Christiana Bauera oraz Gavina Kinga \cite{hibwakcji}.
Jej tytuł może być mylący, jednakże pierwsze jej rozdziały bardzo dokładnie opisują problematykę mapowania obiektowo-relacyjnego i pojęcia z nią związane.

Ponadto liczne źródła elektroniczne, wśród których dominują artykuły naukowe również okazały się pomocne. Odniesienia do nich znajdują się w rozdziale /ref{teoria}, gdzie
pojęcia dotyczące mapowania są objaśniane.
...
{\color{red} Dokładny opis terminologii  pojęć zasadniczych dla tematu pracy, którymi autor będzie się posługiwał przy realizacji głównych celów pracy.}
...
zostać utracone \cite{hibinaction}. W przeciwnym wypadku znalezienie zastosowania dla takich aplikacji byłoby znacznie cięższym zadaniem a rozwiązanie wielu problemów 
...
Duża popularność relacyjnych baz danych wynika z ich licznych zalet, do których należą między innymi łatwość modyfikacji przechowywanych w nich danych, zmniejszona
...
danych \cite{hibinaction}. Relacyjne bazy danych swoje zastosowanie znajdują w wielu systemach i platformach technologicznych, są one najczęściej podstawową 
...
uczynienie tworzonych aplikacji sprawniejszymi i bardziej odpornymi na wszelkiego rodzaju błędy. Szczególnie przydatna może okazać się znajomość właśnie języka SQL,
który jest fundamentem mapowania obiektowo-relacyjnego oraz wszystkich innych aplikacji wykorzystujących relacyjne bazy danych w celu zapewnienia trwałości danych.
...
momentu kiedy przy jej ponownym uruchomieniu nie zajdzie potrzeba jego wczytania. Obiekt jest przechowywany na dysku twardym, a nie jak wcześniej w pamięci operacyjnej,
a ich ilość nie jest ograniczona, ,,utrwalany'' może być pojedyńczy obiekt, ale także całe ich struktury. Warto pamiętać, że większość obiektów nie jest trwała. Obiekty ulotne 
mają ograniczony czas życia, związany z tworzonym je procesem czy też metodą \cite{hibernate}.
...
zarządzanie wszystkimi operacjami oraz unikanie błędów. Decydując się na wykorzystanie relacyjnych baz danych wymienione mechanizmy wykonują za nas sporą część pracy
\cite{hibinaction}.
...
Jak zauważają autorzy książki ,,Hibernate w akcji'' podstawowym problemem w przypadku wykorzystania relacyjnych baz danych oraz programowania obiektowego jest
niedopasowanie paradygmatów. Problem ten może zostać podzielony na kilka mniejszych problemów:

\begin{itemize}
\item Problem szczegółowości -- 
\item Problem podtypów --
\item Problem identyczności --
\item Problemy dotyczące asocjacji --
\item Problem nawizgacji po grafie obiektów --
\end{itemize}

{\color{red} Koszt niedopasowania, warstwa trwałości.}

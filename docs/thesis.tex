...
oraz czy Qubic wnosi coś nowego.
\end{itemize}
...
Rozdział \ref{teoria} zawiera objaśnienia podstawowych zagadnień teotetycznych zwią\-zanych z mapowaniem obiektowo-relacyjnym, czyli między innymi pojęć takich jak
trwałość danych,  relacyjne bazy danych czy też język SQL.
...
architektura aplikacji wysokiego poziomu składa się z trzech warstw: prezentacji (nazywanej też warstwą widoku), logiki oraz trwałości danych.

\begin{figure}[h!]
\centering
\includegraphics[width=.5\textwidth]{resources/layers.png}
...
obiektowe używają w tym celu referencji czy też wskaźników, stąd też łatwo zauważyć, że asocjacje w modelu obiektowym są kierunkowe lub też dwukierunkowe jeśli
...
Relacja wiele do wielu jest tak na prawdę złożeniem dwóch relacji jeden do wielu z dodatkową tabelą łącznikową, nazywaną też asosjacyjną.
...
kierowanych do bazy danych, a więc wykorzystanie mechanizmów złączeń jednakże wydajność nigdy nie będzie taka sama.
\end{itemize}
